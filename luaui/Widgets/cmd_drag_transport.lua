...
function widget:MousePress(mx, my, mButton)
    ...
    if not target or not canTransport(selectedUnit, target) then return false end
...
function widget:MouseRelease(mx, my, mButton)
    ...
    if mButton == 3 and currentTarget and startPos then
        ...
        local pos = mouseToWorldPosition(mx, my)
        Spring.GiveOrder(CMD.LOAD_UNITS, { currentTarget }, (shift and { "shift" } or {}))
        Spring.GiveOrder(CMD.UNLOAD_UNITS, { pos[1], pos[2], pos[3] }, { "shift" })
        endDrag()
    end
end
...
function widget:DrawWorld()
    if startPos and currentPos and currentTarget then
        spEcho("Drawing line from " .. startPos[1]
                .. ", " .. startPos[2]
                .. " to " .. currentPos[1]
                .. ", " .. currentPos[2])
